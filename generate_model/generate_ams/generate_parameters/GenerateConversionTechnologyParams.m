...
technology_max_capacities_non_solar = technologies.conversion_techs_max_capacity(find(~strcmp(technologies.conversion_techs_inputs,'Solar')));
...
if create_param_maximum_capacities == 1
    ...
    for t=1:length(non_solar_energy_conversion_technologies_excluding_grid)
        ...
        definition_string = strcat(definition_string,char(non_solar_energy_conversion_technologies_excluding_grid(t)),':',num2str(technology_max_capacities_non_solar(t)));
    end
    index_domain_string = '';
    for t=1:length(non_solar_energy_conversion_technologies_excluding_grid)
        index_domain_string = strcat(index_domain_string,'''',char(non_solar_energy_conversion_technologies_excluding_grid(t)),'''');
...
params_section = strcat(params_section,param_operating_costs,param_operating_costs_grid,param_OMV_costs,param_linear_capital_costs,param_fixed_capital_costs,...
    param_electricity_feedin_price,param_int_rate,param_lifetimes,param_technology_CRF,param_C_matrix,param_maximum_capacities,param_minimum_part_load);
